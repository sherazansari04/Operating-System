...
process_queue = []
total_wtime = 0
n = int(raw_input('Please Enter the total no of processes: '))
for i in xrange(n):
    process_queue.append([])
    process_queue[i].append(raw_input('Enter process_name: '))
    process_queue[i].append(int(raw_input('Enter process_arrival: ')))
    total_wtime += process_queue[i][1]
    process_queue[i].append(int(raw_input('Enter process_bust: ')))
    print ''
